import Link from "next/link";
import {
  ArrowRight,
  BarChart3,
  Database,
  GitBranch,
  LineChart,
  Search,
  Workflow,
} from "lucide-react";

import { createPageMetadata } from "@/lib/seo";

export const metadata = createPageMetadata(
  "Data Solutions",
  "Veda Solutions Hub is a technology startup building practical data solutions through data engineering, analytics, pipelines, and intelligent systems that turn information into useful insights.",
  "/solutions/data-solutions",
);

const capabilities = [
  {
    icon: Database,
    title: "Data Engineering",
    description:
      "Designing reliable data foundations that make information easier to collect, organize, process, and use.",
  },
  {
    icon: GitBranch,
    title: "Data Pipelines",
    description:
      "Building structured pipelines that move data efficiently between applications, platforms, and storage systems.",
  },
  {
    icon: BarChart3,
    title: "Data Analytics",
    description:
      "Transforming structured information into useful analysis that supports clearer business and operational decisions.",
  },
  {
    icon: LineChart,
    title: "Data Visualization",
    description:
      "Creating meaningful ways to explore and communicate important patterns, trends, and performance indicators.",
  },
  {
    icon: Search,
    title: "Data Insights",
    description:
      "Finding useful patterns and relationships within data to help organizations understand their operations and opportunities.",
  },
  {
    icon: Workflow,
    title: "Intelligent Data Systems",
    description:
      "Connecting data foundations with automation and AI to create more responsive and intelligent digital workflows.",
  },
];

const approach = [
  {
    number: "01",
    title: "Understand",
    description:
      "We identify the available data, its sources, quality, structure, and the decisions or processes it needs to support.",
  },
  {
    number: "02",
    title: "Structure",
    description:
      "We design practical data foundations, pipelines, and systems that make information reliable and accessible.",
  },
  {
    number: "03",
    title: "Turn into insight",
    description:
      "We connect data with analytics, visualization, automation, or AI so it can contribute to meaningful outcomes.",
  },
];

export default function DataSolutionsPage() {
  return (
    <>
      <section className="relative overflow-hidden bg-[#07111F] pt-36 pb-24 sm:pt-40 sm:pb-28">
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute right-[-10%] top-20 h-96 w-96 rounded-full bg-indigo-500/5 blur-3xl" />

          <div className="absolute inset-0 opacity-[0.035] [background-image:linear-gradient(rgba(255,255,255,1)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,1)_1px,transparent_1px)] [background-size:64px_64px]" />
        </div>

        <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
          <div className="max-w-3xl">
            <Link
              href="/solutions"
              className="mb-8 inline-flex items-center gap-2 text-sm font-medium text-slate-500 transition-colors hover:text-teal-400"
            >
              ← All solutions
            </Link>

            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-indigo-400/20 bg-indigo-500/5 px-4 py-2">
              <Database className="h-4 w-4 text-indigo-400" />

              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-400">
                Data Solutions
              </span>
            </div>

            <h1 className="font-[family-name:var(--font-dm-serif)] text-5xl leading-[1.08] tracking-tight text-slate-100 sm:text-6xl lg:text-7xl">
              Turn data into
              <span className="block text-teal-400">useful intelligence.</span>
            </h1>

            <p className="mt-7 max-w-2xl text-base leading-8 text-slate-400 sm:text-lg">
              Strong digital solutions depend on reliable data. We build data
              foundations, pipelines, analytics, and intelligent systems that
              help organizations make better use of the information they
              already have.
            </p>

            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-500 px-6 py-3.5 text-sm font-semibold text-[#07111F] transition-all duration-200 hover:bg-teal-400 hover:shadow-lg hover:shadow-teal-500/20"
              >
                Discuss your data needs
                <ArrowRight className="h-4 w-4" />
              </Link>

              <Link
                href="/solutions/ai-solutions"
                className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#1E334A] bg-white/[0.03] px-6 py-3.5 text-sm font-semibold text-slate-200 transition-all duration-200 hover:border-teal-400/40 hover:bg-teal-500/5 hover:text-teal-400"
              >
                Explore AI solutions
              </Link>
            </div>
          </div>
        </div>
      </section>

      <section className="bg-[#0B1728] py-24 sm:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="max-w-3xl">
            <p className="mb-4 text-sm font-semibold uppercase tracking-[0.18em] text-teal-400">
              What we do
            </p>

            <h2 className="font-[family-name:var(--font-dm-serif)] text-4xl leading-tight tracking-tight text-slate-100 sm:text-5xl">
              Build a stronger foundation for better decisions.
            </h2>

            <p className="mt-5 text-base leading-7 text-slate-400 sm:text-lg">
              We approach data as part of a wider technology ecosystem,
              connecting data engineering and analytics with software, cloud,
              automation, and AI where they create meaningful value.
            </p>
          </div>

          <div className="mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {capabilities.map((capability, index) => {
              const Icon = capability.icon;

              return (
                <div
                  key={capability.title}
                  className="group rounded-2xl border border-[#1E334A] bg-[#07111F] p-7 transition-all duration-300 hover:-translate-y-1 hover:border-teal-400/30 hover:bg-[#0F1F32]"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex h-12 w-12 items-center justify-center rounded-xl border border-teal-400/20 bg-teal-500/10">
                      <Icon className="h-5 w-5 text-teal-400" />
                    </div>

                    <span className="text-xs font-semibold tracking-widest text-slate-600">
                      0{index + 1}
                    </span>
                  </div>

                  <h3 className="mt-7 text-lg font-semibold text-slate-100">
                    {capability.title}
                  </h3>

                  <p className="mt-3 text-sm leading-7 text-slate-400">
                    {capability.description}
                  </p>
                </div>
              );
            })}
          </div>
        </div>
      </section>

      <section className="bg-[#07111F] py-24 sm:py-28">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          <div className="grid gap-14 lg:grid-cols-[0.85fr_1.15fr]">
            <div>
              <p className="mb-4 text-sm font-semibold uppercase tracking-[0.18em] text-teal-400">
                Our approach
              </p>

              <h2 className="font-[family-name:var(--font-dm-serif)] text-4xl leading-tight tracking-tight text-slate-100 sm:text-5xl">
                From raw information to useful outcomes.
              </h2>

              <p className="mt-5 max-w-lg text-base leading-7 text-slate-400">
                Data projects become more valuable when the technology is
                connected to a clear purpose. We focus on building foundations
                that can support both current needs and future systems.
              </p>
            </div>

            <div className="divide-y divide-white/10 rounded-2xl border border-[#1E334A] bg-[#0B1728]">
              {approach.map((step) => (
                <div
                  key={step.number}
                  className="grid gap-5 p-7 sm:grid-cols-[70px_1fr] sm:p-8"
                >
                  <span className="text-sm font-semibold tracking-widest text-teal-400">
                    {step.number}
                  </span>

                  <div>
                    <h3 className="text-xl font-semibold text-slate-100">
                      {step.title}
                    </h3>

                    <p className="mt-3 text-sm leading-7 text-slate-400">
                      {step.description}
                    </p>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      <section className="bg-[#0B1728] py-24 sm:py-28">
        <div className="mx-auto max-w-4xl px-6 text-center lg:px-8">
          <p className="mb-4 text-sm font-semibold uppercase tracking-[0.18em] text-teal-400">
            Data + AI
          </p>

          <h2 className="font-[family-name:var(--font-dm-serif)] text-4xl leading-tight tracking-tight text-slate-100 sm:text-5xl">
            Better data creates better possibilities for intelligent systems.
          </h2>

          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-slate-400 sm:text-lg">
            When reliable data foundations meet AI and automation, information
            can become part of systems that continuously support analysis,
            workflows, and decision-making.
          </p>

          <div className="mt-9 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <Link
              href="/solutions/ai-solutions"
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-500 px-7 py-3.5 text-sm font-semibold text-[#07111F] transition-all duration-200 hover:bg-teal-400 hover:shadow-lg hover:shadow-teal-500/20"
            >
              Explore AI solutions
              <ArrowRight className="h-4 w-4" />
            </Link>

            <Link
              href="/contact"
              className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#1E334A] bg-white/[0.03] px-7 py-3.5 text-sm font-semibold text-slate-300 transition-all duration-200 hover:border-teal-400/30 hover:bg-teal-500/5 hover:text-teal-400"
            >
              Talk to us
            </Link>
          </div>
        </div>
      </section>
    </>
  );
}